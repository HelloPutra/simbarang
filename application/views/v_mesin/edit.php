...
<div class="card-body">
	...
	<form action="<?= base_url(). 'mesin/update'; ?>" method="post">
		<input type="hidden" name="id_bg_mesin" value="<?php echo $bgn->id_bg_mesin ?>">
	  	<div class="form-row">
	  		<div class="col">
           		<label>Nama Mesin</label>
            		<?php foreach ($selected as $sel) { ?>
            			<input type="hidden" name="id_asset" value="<?= $sel->id_asset ?>">
            			<input type="text"class="form-control" value="<?= $sel->id_asset ?> - <?= $sel->nama_asset ?>" readonly>
            		<?php } ?>
	  	    </div>
			<div class="col">
				<label class="control-label">Nama Bagian Mesin</label>
				<input type="text" class="form-control" value="<?= $bgn->nama_bagian ?>" name="nama_bagian" readonly>
			</div>
		</div>
		<div class="form-row">
			<div class="col">
		...
		<div class="modal-footer">
        	<a href="<?= base_url('mesin/detail/').$bgn->id_asset; ?>" class="btn btn-secondary">Batal</a>
